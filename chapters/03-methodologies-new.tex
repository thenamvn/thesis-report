...
\textbf{Google Gemini API Considerations (Cost, Latency, Privacy):} The integration of a cloud-based LLM introduces specific trade-offs. 
\begin{itemize}
    \item \textbf{Cost:} At current usage tiers, Gemini API pricing is manageable for a departmental-scale application, but dynamic scaling costs must be monitored if the platform expands campus-wide.
    \item \textbf{Latency:} LLM generation is inherently slower than standard database queries. To mitigate this, AI generation is processed asynchronously, and the UI provides active loading states to manage user expectations (as noted in NFR1.1, allowing up to 20 seconds for these requests).
    \item \textbf{Data Privacy:} Prompts sent to Gemini contain educational material but are stripped of any Personally Identifiable Information (PII) regarding the students or the institution. However, reliance on a proprietary third-party model means the institution does not have absolute control over the data pipeline, a recognized risk in cloud-based AI integrations.
\end{itemize}
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{Backend (Microservice) (Python / FastAPI):} A specialized service responsible for the complex task of processing `.docx` files. Its sole purpose is to accept a document, extract questions, options, and images, and convert them into a structured JSON format ready for serving. It orchestrates external command-line tools like \textbf{Pandoc} for document structure conversion and \textbf{ImageMagick} for image format transcoding (e.g., WMF/EMF to WebP). The processed output is stored in a shared persistent volume (PV) accessible via HTTP, and its metadata is recorded in the database.

\textbf{Justification for Pandoc over Native XML Parsing:} While parsing the native Office Open XML (OOXML) structure of `.docx` files directly is possible, it is highly complex and error-prone due to the vast and often inconsistent nature of the OOXML specification, especially concerning nested elements like equations (MathML) and embedded objects. Pandoc abstracts this complexity, providing a robust, broadly compatible bridge that reliably converts even heavily formatted academic documents into an intermediate representation (like Markdown or HTML) that is structurally predictable and easier to parse into JSON. This choice significantly reduces development overhead and increases system resilience against malformed or older DOCX variants.
...
The database architecture is built upon a normalized relational model (MySQL) to ensure data integrity, reliable component interactions, and scalability. To enhance clarity and modularity, the system's data model is conceptually divided into four functional modules: User \& Security, Room \& Content, User Interaction \& Outcomes, and Rewards \& Utilities.

\subsubsection{User and Security Module}
This module forms the backbone of identity management and social features. It centers on the `USERS` table, which stores core credentials and statistics. 
\begin{itemize}
    \item \textbf{User Profiles:} The `USER_PROFILE` table, maintaining a 1-to-1 relationship with `USERS`, stores extended personal information (avatar, bio, social links).
    \item \textbf{Social Graph:} The `USER_FOLLOWS` table implements a many-to-many self-referencing relationship to manage follower/following dynamics between users.
    \item \textbf{Security:} The `OTP` table handles temporary tokens for account verification, while the isolated `ADMIN_ACCOUNT` table secures privileged access.
\end{itemize}
This structure ensures separation of critical authentication data from public profile information, as illustrated in Figure \ref{fig:erd-user-security}.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/erd-user-security.png}
\caption{ER Diagram: User and Security Module}
\label{fig:erd-user-security}
\end{figure}
\FloatBarrier

\subsubsection{Room and Content Management Module}
This module manages the core educational content. The central entity is `ROOM`, which can be of type 'quiz' or 'test'.
\begin{itemize}
    \item \textbf{Room Ownership:} A host (`USERS`) manages multiple `ROOM`s (1-to-Many).
    \item \textbf{Questions Repository:} Each `ROOM` contains multiple `QUESTIONS`. Questions support diverse types (Multiple Choice, Text, etc.) and store the correct answer hash or text.
    \item \textbf{Option Banks:} For multiple-choice questions, the `QUESTION_OPTIONS` table stores possible answers, linked via a foreign key to the parent question.
\end{itemize}
This hierarchy allows for structured content organization, supporting both manual creation and AI-generated imports (Figure \ref{fig:erd-room-content}).

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/erd-room-content.png}
\caption{ER Diagram: Room and Content Management}
\label{fig:erd-room-content}
\end{figure}
\FloatBarrier

\subsubsection{Interaction and Assessment Module}
This module tracks the dynamic interactions between users and the content, recording performance and integrity data.
\begin{itemize}
    \item \textbf{Submissions:} The `USER_SUBMISSIONS` table serves as a ledger, recording every answer attempt a user makes for a specific question within a room.
    \item \textbf{Exam Results:} For the specific use case of Test Rooms (`TEST_EXAM_ROOMS`), the `EXAM_RESULTS` table aggregates the final performance metrics (score percentage, correct count).
    \item \textbf{Integrity Logs:} The `activity_log` JSON field within `EXAM_RESULTS` and related tables stores timestamped security events (tab switching, focus loss) detected by the anti-cheat system.
\end{itemize}
Figure \ref{fig:erd-interaction-results} visualizes how user actions map to persistent records.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{figures/erd-interaction-results.png}
\caption{ER Diagram: Submissions, Exams, and Integrity}
\label{fig:erd-interaction-results}
\end{figure}
\FloatBarrier

\subsubsection{Rewards and Utilities Module}
This section handles the gamification and auxiliary system features.
\begin{itemize}
    \item \textbf{Voucher System:} The `VOUCHERS` table stores digital rewards. These are distributed by `ROOM`s and claimed by `USERS` (`USER_VOUCHERS`). Additionally, specific questions can trigger voucher drops (`QUESTION_VOUCHERS`).
    \item \textbf{Reporting:} The `REPORTS` table allows users to flag inappropriate rooms, facilitating community moderation.
    \item \textbf{Media Gallery:} The `IMAGES` table tracks files uploaded by hosts, linking them to specific rooms for easy retrieval and cleanup.
\end{itemize}
This modular design (Figure \ref{fig:erd-rewards-utilities}) allows the gamification layer to function independently without cluttering the core assessment logic.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/erd-rewards-utilities.png}
\caption{ER Diagram: Rewards and System Utilities}
\label{fig:erd-rewards-utilities}
\end{figure}
\FloatBarrier
...
\renewcommand{\arraystretch}{1.5}
\begin{longtable}{p{1.2cm} p{7.5cm} p{4.3cm} p{2.0cm}}
...
\texttt{POST} & \texttt{/auth/register} & Register a new user. & No \\
\texttt{POST} & \texttt{/auth/login} & Authenticate a user and return a JWT. & No \\
\texttt{POST} & \texttt{/api/rooms} & Create a new Quiz or Test Room. & Yes \\
\texttt{POST} & \texttt{/api/rooms/generate-ai} & Generate questions for a room using AI. & Yes \\
\texttt{POST} & \texttt{/api/v1/process-docx} & Upload a DOCX file for processing by the Python microservice. & Yes \\
\texttt{GET} & \texttt{/api/rooms/:roomCode} & Get public details for a specific room. & No \\
\texttt{POST} & \texttt{/api/rooms/:roomCode/submit} & Submit an answer to a question. & Yes \\
\texttt{GET} & \texttt{/api/discovery/nearby} & Find nearby rooms using GPS coordinates. & No \\
\texttt{POST} & \texttt{/api/vouchers/verify} & Verify a voucher via its QR code data. & Yes (Host) \\

\end{longtable}
\FloatBarrier

\subsection{Python Microservice for DOCX Processing}
\label{subsec:python-microservice}

The DOCX processing microservice is a sophisticated backend component developed using Python and FastAPI. It serves as the engine for transforming static instructor-uploaded documents into interactive, secure assessments. Beyond simple file parsing, this microservice manages the entire lifecycle of an exam generated from a document, including creation, delivery, security validation, and results processing. Containerized via Docker, it operates independently of the main Node.js backend, ensuring that CPU-intensive parsing tasks do not degrade the performance of real-time user interactions.

\subsubsection{Rationale: The Decision for a LaTeX-Intermediary Pipeline}
A critical architectural decision was choosing how to reliably extract text, mathematical formulas, and images from `.docx` files. While `.docx` is intrinsically a zipped collection of XML files (Office Open XML - OOXML), parsing the raw `document.xml` was ultimately rejected in favor of a `DOCX -> LaTeX -> JSON` pipeline utilizing Pandoc. This decision was driven by four major technical challenges:

\begin{enumerate}
    \item \textbf{Mathematical Formulas and OLE Objects:} In standard DOCX files, legacy mathematical formulas (e.g., from MathType) are often stored as binary Objects Object Linking and Embedding (OLE) data (`oledata.mso`), which are unreadable via standard XML parsing. Pandoc excels at interpreting these binaries and automatically extracting their fallback standard vector graphics (\texttt{.wmf} format). Attempting to manually parse the OOXML to extract and decode these binary structures is highly error-prone and inefficient.
    \item \textbf{OOXML Structural "Noise":} The native OOXML generated by Microsoft Word contains excessive formatting tags (e.g., a single word can be fragmented into multiple `<w:r>` tags due to typing history or minor styling changes). Pandoc acts as a sanitizer, stripping this "XML noise" and normalizing the document into a clean, structurally sound LaTeX format.
    \item \textbf{Parsing Complexity:} LaTeX is plain text with a linear, flat structure. Writing Regular Expressions (Regex) and custom string parsing logic in Python to convert LaTeX text into structured JSON is significantly more straightforward and maintainable than traversing the deeply nested and complex DOM tree of Microsoft's OOXML.
    \item \textbf{Web Performance Optimization:} The system intercepts the extracted \texttt{.wmf} (Windows Metafile) assets—which cannot be rendered natively by web browsers (ReactJS)—and immediately transcodes them into highly optimized \texttt{.webp} images before mapping them to the final JSON structure. This demonstrates a clear separation of concerns, offloading formatting normalization to Pandoc while keeping web optimization logic in the backend.
\end{enumerate}

\subsubsection{The DOCX-to-JSON Pipeline}

The core functionality of the microservice is the transformation of unstructured DOCX files into a structured, machine-readable JSON format used by the frontend. This pipeline is orchestrated by the \texttt{DocxService} class, which manages a strict sequence of operations to ensure data integrity and format consistency. The process is visualized in Figure \ref{fig:python-microservice-flow}.

\begin{figure}[htbp]
\centering
\includegraphics[height=0.7\textwidth]{figures/python-microservice-flow.png}
...
\begin{enumerate}
    \item \textbf{Ingestion and Validation:} The service accepts a multipart/form-data upload containing the `.docx` file. It validates the file type and initializes a unique processing context identified by a UUID.
    \item \textbf{Intermediate Conversion:} \texttt{Pandoc} is utilized to convert the binary DOCX format into LaTeX. This intermediate step is crucial as it normalizes the document structure (sections, lists, bold/italic text) and simplifies the extraction of embedded questions.
    \item \textbf{Media Extraction and Optimization:} During conversion, all embedded images are extracted. The \texttt{ImageUtils} module subsequently scans these assets, identifying legacy formats often found in Word documents (e.g., WMF, EMF). It uses \texttt{ImageMagick} to transcode these into high-performance WebP format, optimizing them for web delivery. This process runs in parallel threads to minimize latency.
    \item \textbf{Semantic Parsing:} A custom parser analyzes the LaTeX output to identify question blocks, distinguishing between question text, options, and correct answers marked by the instructor (e.g., via bolding or specific formatting).
    \item \textbf{Resource Mapping:} Extracted image references in the text are dynamically replaced with public URLs pointing to the service's static file server.
    \item \textbf{Persistence:} The final JSON structure is saved to disk, and a record is created in the database, marking the exam as ready for access.
\end{enumerate}

The orchestration logic is encapsulated in the \texttt{DocxService} class, as shown in Listing \ref{lst:docx-service}.

\begin{lstlisting}[language=Python, caption={Core Logic of the DocxService Orchestrator}, label={lst:docx-service}]
import pypandoc
import os
...
class DocxService:
    def process_docx(self, input_path: str, uuid: str) -> dict:
        ...
        DatabaseService.create_test_exam_room(uuid=uuid, title="Generated Exam")

        return {"status": "success", "uuid": uuid}

    def convert_docx_to_latex(self, input_path, output_path, media_dir):
        # Wrapper for pypandoc call with media extraction
...
\subsubsection{API Interface and Lifecycle Management}

The microservice exposes a comprehensive set of RESTful endpoints that manage the entire lifecycle of a document-based exam. These endpoints govern the process from initial document ingestion to final result analytics, ensuring secure and stateful interactions. The key lifecycle management endpoints are detailed in Table \ref{tab:microservice-api-lifecycle}.
\renewcommand{\arraystretch}{1.25}
\begin{longtable}{P{2.5cm} P{6.5cm} P{6.5cm}}
\caption{Microservice API Endpoints for Exam Lifecycle Management} 
\label{tab:microservice-api-lifecycle} \\

\toprule
\textbf{Lifecycle Stage} & \textbf{Endpoint / Method} & \textbf{Functionality \& Security Measures} \\
\midrule
\endfirsthead

\multicolumn{3}{l}{\tablename\ \thetable{}: Microservice API Endpoints \textit{(continued)}} \\[0.25em]
\toprule
\textbf{Lifecycle Stage} & \textbf{Endpoint / Method} & \textbf{Functionality \& Security Measures} \\
\midrule
\endhead
...
\textbf{1. Generation} &
\texttt{POST} \newline
\texttt{/api/v1/process-docx} &
Accepts a raw DOCX file and metadata. Initiates the conversion pipeline and returns a unique \texttt{UUID} identifying the newly created exam room. \\

\midrule
\textbf{2. Delivery} &
\texttt{GET} \newline
\texttt{/api/v1/quiz/\{uuid\}} &
Retrieves exam content for students.
\textbf{Security:} Server-side filtration removes all correct-answer markers from the JSON response to prevent client-side inspection. \\

\midrule
\textbf{3. Submission} &
\texttt{POST} \newline
\texttt{/api/v1/quiz/check-answers} &
Receives student answers and proctoring logs. Performs server-side scoring using the secure answer key and analyzes security logs to detect violations. \\

\midrule
\textbf{4. Analytics} &
\texttt{GET} \newline
\texttt{/api/v1/quiz/\{uuid\}/results} &
Provides aggregate results, leaderboards, and detailed security incident reports for instructors to assess performance and academic integrity. \\
...
Academic integrity in online testing presents unique challenges compared to traditional proctored exams. The TEKUTOKO platform implements a client-side, browser-based anti-cheat system designed specifically for Test Rooms. 

\subsubsection{Rationale for Client-Side Implementation}
The decision to implement proctoring primarily on the client side (via modern Web APIs) rather than relying on server-side video streaming or invasive desktop software was deliberate. It is an approach rooted in balancing educational integrity with student privacy. By monitoring browser states (visibility, focus, and DOM events) rather than capturing audio/video or recording keystrokes, the system offers a \textbf{non-invasive} solution. It establishes a strong psychological deterrent for low-to-medium stakes assessments without exposing students to the privacy risks typically associated with heavy surveillance tools.
...
\begin{itemize}
    \item \textbf{JWT-Based Authentication:} The system uses JSON Web Tokens (JWT) for stateless authentication. After a successful login, the client receives an access token which is sent in the `Authorization` header of subsequent requests. The flow is visualized in Figure \ref{fig:jwt-flow}.
    
    \item \textbf{Password Encryption:} User passwords are never stored in plaintext. They are hashed using the \texttt{bcrypt} algorithm with a work factor (salt rounds) of 10. This ensures that even if the database is compromised, the original passwords remain protected against rainbow table and brute-force attacks.

    \item \textbf{OTP Security:} One-Time Passwords (OTPs) used for email verification and password resets are generated using a cryptographically secure random number generator. To prevent abuse, OTPs are: 
    \begin{itemize}
        \item \textbf{Time-bound:} Valid for only 5 minutes.
        \item \textbf{Single-use:} Invalidated immediately after successful verification.
        \item \textbf{Rate-limited:} Requests are throttled to prevent spamming.
    \end{itemize}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.6\textwidth]{figures/jwt-flow.png}
...
A state-of-the-art large language model used for the AI Question Generator, capable of understanding complex prompts and producing high-quality, structured JSON output \citep{gemini2023}. \\[0.5em]

\textbf{Deployment} & \textbf{Kubernetes (K8s), Docker} &
Docker containers package all services, while a Kubernetes cluster orchestrates deployment, scaling, and networking (Ingress), ensuring high availability and robust resource management. \\[0.5em]
...
\end{itemize}
\input{chapters/03-testing-methodology}
\input{chapters/devops-deployment}
